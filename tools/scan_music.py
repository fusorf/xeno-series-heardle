...
Reads ID3 metadata (title, artist, composer) directly from MP3 files.
"""
...
def read_metadata(filepath):
    """Read ID3 metadata from an MP3 file. Returns (title, artist, composer, duration)."""
    try:
        audio = MP3(filepath)
        ...
        if tags:
            ...
            # TCOM = composer
            if 'TCOM' in tags:
                composer = str(tags['TCOM'])

        return title, artist, composer, duration
    except Exception as e:
        print(f"    [!] Error reading {filepath}: {e}")
        return None, None, None, 180
...
def scan_folder(folder_path, game_id):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in sorted(files):
            ...
            # Read ID3 metadata
            title, artist, composer, duration = read_metadata(filepath)

            # Fallback: use filename if no TIT2 tag
            if not title:
                title = os.path.splitext(file)[0]
                print(f"    [!] No TIT2 tag for {file}, using filename")
...
def generate_js_array(game_id, songs):
    ...
    for song in songs:
        ...
        if "composer" in song:
            js_code += f',\n    "composer": {json.dumps(song["composer"], ensure_ascii=False)}'
        if "artist" in song:
